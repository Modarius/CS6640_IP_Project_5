...
def connectedComponents(image, min_size=5):
    out_im = np.zeros_like(image,dtype=int)
    image = morph.remove_small_objects(image, min_size=min_size)
    image = morph.remove_small_holes(image, area_threshold=min_size)
...
def ocr(image):
    ...
    denoised_im = restore.denoise_bilateral(image)
    threshold_im = np.round(restore.denoise_nl_means(denoised_im < filters.threshold_otsu(denoised_im))).astype(bool)
    ...
    while(process_props.__len__() > 0):
        ...
        tl_props = dict(process_props.loc[idx_tl])
        current_row = process_props.query("min_row <" + str(tl_props['max_row'])) # get the characters in the current row
        ...
        for idx in current_row.index:
            ...
            scaled_img = transform.rescale(curr_img, 28 / np.array(curr_img.shape).max(), anti_aliasing=False)
            ...
            if(scaled_img.shape != (28, 28)):
                ...
                pad_x_begin = np.floor((28 - col)/2).astype(np.int8)
                pad_x_end = np.ceil((28 - col)/2).astype(np.int8)
                pad_y_begin = np.floor((28-row)/2).astype(np.int8)
                pad_y_end = np.ceil((28-row)/2).astype(np.int8)
            padded_img = np.pad(scaled_img, pad_width=((pad_y_begin, pad_y_end),(pad_x_begin, pad_x_end)))
            char_img = (padded_img > 0).astype(np.int8)
            ax1 = plt.subplot(2, len(current_row), i)
            ax2 = plt.subplot(2, len(current_row), i + len(current_row))
            ax1.imshow(char_img)
            ax2.imshow(curr_img)
